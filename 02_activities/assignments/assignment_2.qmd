...
```{r}
system("head ./02_activities/data/gwa.A2.fam")

# To capture the output of wc -l specifically
line_count <- system("wc -l ./02_activities/data/gwa.A2.fam", intern = TRUE)
print(line_count)
```
...
```{r}
system("head ./02_activities/data/gwa.A2.bim")

# To capture the output of wc -l specifically
line_count <- system("wc -l ./02_activities/data/gwa.A2.bim", intern = TRUE)
print(line_count)
```
...
```{bash}
plink2 --bfile 02_activities/data/gwa.qc_unrelated.A2 --extract ./02_activities/data/gwa.qc.A2.prune.in --make-bed --out ./02_activities/data/gwa.qc_unrelated_pruned_A2
```


```{bash,results='hide',message=FALSE, warning=FALSE}
plink2 --bfile ./02_activities/data/gwa.qc_unrelated_pruned_A2 --pca 20 --out ./02_activities/data/gwa_unrel_PCA_A2
```
...
```{bash}
plink2 --bfile ./02_activities/data/gwa.qc_unrelated.A2 --ci 0.95 --glm --covar ./02_activities/data/gwa_unrel_PCA_A2.eigenvec --covar-name PC1,PC2,PC3 --adjust --out ./02_activities/data/gwa.qc_assoc_A2
```
...
assoc <- data.table::fread("./02_activities/data/gwa.qc_assoc_A2.PHENO1.glm.linear")

head(assoc)

assoc_adjusted <- data.table::fread("./02_activities/data/gwa.qc_assoc_A2.PHENO1.glm.linear.adjusted")
head(assoc_adjusted)
```
