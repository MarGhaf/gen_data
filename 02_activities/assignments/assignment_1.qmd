...
plink2 --bfile ./02_activities/data/gwa.qc.A1 \
      ...
      --recode A \
      --keep ./02_activities/data/samplist.txt \
...
```{bash}
# Calculate frequencies for the 4 specific SNPs
plink2 --bfile ./02_activities/data/gwa.qc.A1 \
       --extract ./02_activities/data/snplist.txt \
       --freq \
       --out ./02_activities/data/gwa_qc_freq
```
...
```         
The beta coefficient of 0.967 indicates a positive additive effect. Each additional copy of the C allele at SNP rs1861 results in a predicted increase of 0.967 units in the phenotype.
```
...
```         
the  recessive model fits better because the regression line accurately passes through the center of the data clusters for both categories. In contrast, the additive model exhibits poor fit at the '2' genotype level, where the predicted value is significantly higher than the actual observed phenotype, leading to larger residuals and a biased estimate of the allelic effect
```
